...
public class AssignInGameFlows extends BaseFlows<ValidateContext> {
   ...
   public AssignInGameFlows(RSLBCore core) {
      this.core = core;
   }
      ...
      try {
         ...
         if (inGameUUID == null) {
            ...
            synchronized (AssignInGameFlows.class) {
               while (this.core.getSqlManager().getInGameProfileTable().dataExists(inGameUUID)) {
                  this.core.getLogger().warning(String.format("UUID %s has been used and will take a random value.", inGameUUID.toString()));
         ...
         if (this.core.getPluginConfig().isNameCorrect()) {
            ...
            if (modified) {
               ...
               this.core.getLogger().warning(String.format("The name %s is occupied, change it to %s.", initFixName, fixName));
               Bukkit.getAsyncScheduler()
                  .runDelayed(
                     this.core.getPlugin(),
                     task -> {
                        Player player = Bukkit.getPlayer(finalInGameUUID);
                        if (player != null) {
