...
import com.rserene.chosen.server.http.LoggingInterceptor;
import com.rserene.chosen.server.http.RetryInterceptor;
   ...
   public SkinRestorerResultImpl doRestorer(GameProfile profile, BaseServiceConfig serviceConfig) {
      try {
         ...
         OkHttpClient okHttpClient = new Builder()
            .addInterceptor(new RetryInterceptor(this.core, serviceConfig.getSkinRestorer().getRetry(), serviceConfig.getSkinRestorer().getRetryDelay()))
            .addInterceptor(new LoggingInterceptor(this.core))
         ...
         if (isSignatureValid(textures.getValue(), textures.getSignature())) {
            ...
            this.core.getLogger().warning(profile.getName() + " has a valid skin signature, but the skin URL is invalid.");
         }
